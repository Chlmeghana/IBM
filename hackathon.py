        ...
        """if not quiet:
            for line in non_empty[:-1]:
                if line != 80*" ":
                    print("-->",line)"""
        return non_empty[:-1]
    def edl_compile(self,fields_list,index_number, flag= None, quiet=False):
        s = self.save_screen_string()
        non_empty = list(filter(None, s.data))
        if not quiet:
            for line in non_empty[:-1]:
...
class console:
    ...
    def edl_compile_function(self,fields):
        self.em.send_clear()
        s = self.em.save_screen_string()
        c=0
        flag=0
        ...
        while True:
            time.sleep(1)
            screen_lines = self.em.edl_compile(fields,index_number,quiet=self.args['quiet'])
            index_number+=1
...
def main():
    ...
    try:
        ...
        result1=c.edl_compile_function(fields)
        print(result1)
        c.logoff()
